import { CustomProductT } from "@/shared/types/printify";
import { GET } from "./apiClient";
import {
    CreatePrintifyOrderRequestSchema,
    CustomProductResponseSchema,
    PrintifyOrderResponseSchema,
} from "@/shared/schemas/services/printifyServiceSchemas";
...
export class PrintifyService {
    ...
    static async getCustomProduct(productId: string): Promise<CustomProductT> {
        try {
            ...
            const validatedResponse = CustomProductResponseSchema.parse(
                response,
            );
    ...
    static async createPrintifyOrder(
        ...
        try {
            ...
            const validatedPayload = CreatePrintifyOrderRequestSchema.parse(
                payload,
            );
            ...
            if (isProduction && isTestOrder) {
                ...
                console.error(
                    "❌ CRITICAL: Attempted to create test order in production",
                );
                throw new Error(
            ...
            if (isTestOrder) {
                console.log(
                    "🧪 Creating TEST Printify order (will not be sent to production)",
                );
            } else {
                console.log(
                    "🚀 Creating PRODUCTION Printify order (will be manufactured and shipped)",
                );
            }
            ...
            if (sessionError || !session) {
                throw new Error(
                    "You must be logged in to create a Printify order",
                );
            }
            ...
            if (!data) {
                throw new Error(
                    "No response data from Printify order creation",
                );
            }
     ...
     */
    static async getTshirtProducts(
