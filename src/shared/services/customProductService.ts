...
} from "@/shared/types/customProduct";
import {
...
} from "@/shared/schemas/customProduct";
import { CustomProductServiceMapper } from "@/shared/mappers/services/customProductServiceMapper";
...
export class CustomProductService {
  ...
  static async uploadImage(
    ...
    { id: string; previewUrl: string; width: number; height: number }
  > {
    try {
      ...
      const payload = CustomProductServiceMapper.mapImageUrlToUploadRequest(
        imageUrl,
      );
  ...
  static async createCustomProduct(
    ...
    try {
      ...
      if (!accessToken) {
        throw new Error(
          "Authentication required. Please log in to create products.",
        );
      }
      ...
      if (validatedInput.print_positions?.length) {
        ...
        if (productConfig.disablePlacementAdjustment) {
          ...
          const defaultPosition = productConfig.defaultPosition ||
            positions[0] || "front";
      ...
      const validatedProductPayload = CreateCustomProductRequestSchema.parse(
        productPayload,
      );
      ...
      if (!validatedResponse.success || !validatedResponse.product) {
        throw new Error(
          validatedResponse.error || "Failed to create custom product",
        );
      }
