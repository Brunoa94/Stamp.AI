...
export class PaymentRecoveryService {
  ...
  static async recordPaymentForRecovery(
    ...
    try {
      ...
      if (!user) {
        console.warn(
          "⚠️ Cannot record payment recovery - user not authenticated",
        );
        return null;
      }
      ...
      const { data, error } = await supabase.rpc(
        "record_payment_for_recovery",
        {
          ...
          p_cart_snapshot: payload.cartSnapshot as unknown as Record<
            string,
            unknown
          >,
          p_shipping_address: payload.shippingAddress as unknown as Record<
            string,
            unknown
          >,
          p_line_items: payload.lineItems as unknown as Record<
            string,
            unknown
          >[],
