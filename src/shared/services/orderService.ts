...
import { OrderSchema, OrderWithItemsSchema } from "@/shared/schemas/order";
...
import { z } from "zod";
import { CartItem, CartT, CartWithItems } from "@/shared/types/cart";
...
export class OrderService {
  ...
  static async getOrder(orderId: string): Promise<OrderWithItemsT> {
    try {
      ...
      if (!data) {
        throw ErrorClient.handleError({
          error: new Error(`Order not found with id: ${orderId}`),
          service: "Order",
          action: "Get Order",
        });
      }
    ...
    try {
      ...
      if (error) {
        throw ErrorClient.handleError({
          error,
          service: "Order",
          action: "Get Order By Number",
        });
      }

      if (!data) {
        throw ErrorClient.handleError({
          error: new Error(`Order not found with number: ${orderNumber}`),
          service: "Order",
          action: "Get Order By Number",
        });
      }
  ...
  static async createOrder(payload: CreateOrderT): Promise<OrderT> {
    try {
      ...
      if (!data) {
        throw ErrorClient.handleError({
          error: new Error("No data returned after order creation"),
          service: "Order",
          action: "Create Order",
        });
      }
  ...
  static async updateOrder(
    ...
    try {
      ...
      if (!data) {
        throw ErrorClient.handleError({
          error: new Error(
            `Order not found or update failed for id: ${orderId}`,
          ),
          service: "Order",
          action: "Update Order",
        });
  ...
  static async updatePaymentStatus(
    ...
    try {
      const updatePayload = OrderServiceMapper.mapPaymentStatusToUpdate(
        paymentStatus,
      );
      return await this.updateOrder(orderId, updatePayload);
    } catch (error) {
  ...
  static async updateTracking(
    ...
    try {
      const updatePayload = OrderServiceMapper.mapTrackingToUpdate(
        trackingNumber,
        trackingUrl,
      );
      return await this.updateOrder(orderId, updatePayload);
  ...
  static async linkPaymentTransactionToOrder({
    ...
    try {
      ...
      if (error) {
        ...
        console.log(
          `✅ Linked payment transaction (${paymentProvider}:${paymentIntentId}) to order ${orderId}`,
        );
      }
    } catch (error) {
  ...
  static async createOrderFromCart({
    ...
    try {
      ...
      if (idempotencyKey) {
        const existingOrder = await this.getOrderByIdempotencyKey(
          idempotencyKey,
        );
        if (existingOrder) {
          console.log(
            `⚠️ Order already exists with idempotency key: ${idempotencyKey}, returning existing order: ${existingOrder.id}`,
          );
  ...
  static async handlePrintifyFailure({
    ...
    try {
      console.error(
        `❌ Printify order creation failed for order ${orderId}:`,
        printifyError,
      );
      ...
      try {
        ...
        console.error(
          `Failed to update order status to unsuccessful_confirmation:`,
          statusError,
        );
      ...
      try {
        ...
        console.error(
          `❌ Refund initiation failed for order ${orderId}:`,
          refundError,
        );
