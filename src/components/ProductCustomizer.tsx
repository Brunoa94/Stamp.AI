...
import ImageUploader, { PrintifyImage } from "./ImageUploader";
import {
  BLUEPRINTS,
  Blueprint,
  getBlueprintById,
} from "@/lib/printify/blueprints";
...
export default function ProductCustomizer({
  ...
  const handleFrontImageUpload = (image: PrintifyImage) => {
    setUploadedFrontImage(image);
    setError(null);
  };

  const handleBackImageUpload = (image: PrintifyImage) => {
    setUploadedBackImage(image);
    setError(null);
  };
      ...
      <div className="bg-white border rounded-lg p-6">
        ...
        {activePrintSide === "front" && (
          <div>
            ...
            <ImageUploader
              onUploadComplete={handleFrontImageUpload}
        ...
        {activePrintSide === "back" && (
          <div>
            ...
            <ImageUploader
              onUploadComplete={handleBackImageUpload}
              onError={(err) => setError(err)}
            />
            {uploadedBackImage && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
