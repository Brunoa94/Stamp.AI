...
DEFAULT_PROFILE_PATH = "/Users/brunoafonso/Downloads/profiling-data.26-03-2026.22-40-05.json"

ROUTE_KEYWORDS = {
    "orders": ("Orders", "Order", "my-orders", "orders"),
    "stamp": (
        "Wizard",
        "CreateProduct",
        "Stamp",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze React profiler export")
    parser.add_argument(
        "--profile",
        default=DEFAULT_PROFILE_PATH,
        help="Path to profiling-data JSON file",
    )
    parser.add_argument(
        "--route",
        choices=["all", "orders", "stamp"],
        default="all",
        help="Filter commits by route-related updater names",
    )
    return parser.parse_args()


def get_route_commits(commits: list[dict], route: str) -> list[dict]:
    if route == "all":
        return commits
    ...
    for commit in commits:
        updater_names = [
            updater.get("displayName") or ""
            for updater in (commit.get("updaters") or [])
        ]
        joined = " ".join(updater_names)
...
def main() -> None:
    ...
    commits = get_route_commits(root["commitData"], args.route)

    if not commits:
        print("COMMITS", 0)
        print("No commits found for selected route filter.")
        return
    ...
    for pctl in (90, 95, 99):
        idx = min(
            ...
            int(round((pctl / 100) * (len(sorted_durations) - 1))),
        )
    ...
    for commit in sorted(commits, key=lambda item: item["duration"], reverse=True)[:12]:
        updaters = sorted(
            {
                (
                    updater.get("displayName")
                    or f"id:{updater.get('id')}"
                )
                for updater in (commit.get("updaters") or [])
            }
        )
        print(
            round(commit["duration"], 2),
            commit.get("priorityLevel"),
            "updaters=",
            ",".join(updaters),
        )
    ...
    for fiber_id, total in sorted(
        ...
        print(
            fiber_id,
            name_by_id.get(fiber_id, f"id:{fiber_id}"),
            round(total, 2),
            round(fiber_self[fiber_id], 2),
            render_count[fiber_id],
        )
    ...
    effects = [
        commit.get("effectDuration")
        for commit in commits
        if commit.get("effectDuration") is not None
    ]
    passive_effects = [
        commit.get("passiveEffectDuration")
        for commit in commits
        if commit.get("passiveEffectDuration") is not None
    ]

    print(
        "EFFECT_SUM_MAX",
        round(sum(effects), 2) if effects else 0,
        max(effects) if effects else 0,
    )
    print(
        "PASSIVE_SUM_MAX",
        round(sum(passive_effects), 2) if passive_effects else 0,
        max(passive_effects) if passive_effects else 0,
    )
