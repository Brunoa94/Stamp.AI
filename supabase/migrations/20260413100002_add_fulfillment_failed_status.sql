-- =====================================================
-- Add 'fulfillment_failed' Order Status
-- Created: 2026-04-13
-- Description: Adds new status for orders where Printify creation fails
--              These orders should be immediately refunded
-- =====================================================
...
-- Add new check constraint with 'fulfillment_failed' status
ALTER TABLE orders
  ADD CONSTRAINT check_valid_order_status
  CHECK (status IN ('pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'fulfillment_failed'));

COMMENT ON CONSTRAINT check_valid_order_status ON orders IS
  'Valid order statuses:
   - pending: Order created, awaiting fulfillment
   - confirmed: Payment received and Printify order created
   - processing: Printify is manufacturing
   - shipped: Product shipped to customer
   - delivered: Product delivered
   - cancelled: Order cancelled by customer or system
   - fulfillment_failed: Printify order creation failed, refund processed';

-- Update the atomic function to accept the new status
...
BEGIN
  ...
  IF p_order_status IS NOT NULL AND
     p_order_status NOT IN ('pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'fulfillment_failed') THEN
...
COMMENT ON FUNCTION update_order_payment_status_atomic IS
  'Atomically updates payment_status and optionally status for an order.
   Webhooks should call with only payment_status (leave order_status NULL).
   Fulfillment service should update order_status after operations complete.';
